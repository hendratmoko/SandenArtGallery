...
/**
 * FUNGSI BANTU UNTUK TESTING MANUAL DARI EDITOR APPS SCRIPT.
 * Ini AMAN dipilih & di-klik "Run ▶" langsung di editor (tidak seperti
 * doPost/doGet yang butuh request HTTP asli). Fungsi ini akan:
 * 1. Menulis 1 baris data uji ke sheet.
 * 2. Membaca kembali seluruh data lewat doGet.
 * Lihat hasilnya di menu View > Logs (atau Executions) setelah Run.
 */
function testSimpanData() {
  ...
  Logger.log('Hasil doPost: %s', result.getContent());

  const getResult = doGet();
  const parsed = JSON.parse(getResult.getContent());
  Logger.log('Total baris data setelah tes: %s', parsed.data ? parsed.data.length : 'gagal, error: ' + parsed.error);
}
 ...
 *
 * ⚠️ CATATAN TESTING: jangan menguji fungsi ini dengan tombol "Run ▶"
 * di editor Apps Script. Saat di-Run manual, Google TIDAK mengirim
 * objek request (e) sama sekali, sehingga "e" bernilai undefined —
 * ini situasi yang tidak pernah terjadi pada permintaan asli dari
 * website (browser selalu mengirim e.postData saat POST). Untuk
 * menguji secara nyata, gunakan tombol Register/Upload di website,
 * atau kirim POST lewat curl/Postman ke URL .../exec.
 */
function doPost(e) {
  // Lapisan pengaman terluar: apapun yang terjadi di dalam (termasuk
  // kalau "e" undefined/tidak lengkap), fungsi ini SELALU mengembalikan
  // JSON yang valid, tidak pernah melempar error mentah ke pemanggil.
  try {
    return handleCreateRequest_(e);
  } catch (err) {
    return jsonOut_({ isOk: false, error: 'doPost error: ' + String(err) });
  }
}

function handleCreateRequest_(e) {
  // "e" bisa undefined kalau fungsi dipanggil manual (bukan lewat HTTP
  // request sungguhan). Selalu jadikan objek kosong dulu supaya baris
  // berikutnya aman diakses.
  e = e || {};

  if (!e.postData || !e.postData.contents) {
    return jsonOut_({
      isOk: false,
      error: 'Request tidak berisi data (postData kosong). Ini normal jika fungsi diuji lewat tombol "Run" di editor — coba submit dari form Register/Upload di website, atau kirim POST asli ke URL /exec.'
    });
  }
